#!/usr/bin/env python3
"""
昱金生能源集團 - 員工日報分析報告生成器 V2
使用 reportlab 生成優化的 PDF 報告（支援中文）
"""
...
def create_pdf():
    # 建立 PDF 文件
    output_path = "/home/yjsclaw/.openclaw/workspace/reports/2026-02-25_employee_report_v2.pdf"
    ...
    content.append(Paragraph("Employee Daily Report Analysis - 2026/02/25", 
                            ParagraphStyle('temp', parent=normal_style, alignment=TA_CENTER, fontSize=10)))
    ...
    for i, emp in enumerate(EMPLOYEE_REPORTS, 1):
        # 員工卡片標題
        score_color = colors.HexColor('#48bb78') if emp["score"] >= 90 else colors.HexColor('#4299e1') if emp["score"] >= 85 else colors.HexColor('#ed8936') if emp["score"] >= 80 else colors.HexColor('#f56565')
