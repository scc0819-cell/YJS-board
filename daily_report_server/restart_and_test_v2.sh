...
# 2. 刪除 auth_state（在啟動前！）
echo "2️⃣  清除登入鎖定狀態..."
rm -f /home/yjsclaw/.openclaw/workspace/daily_report_server/data/auth_state.json
echo "✅ auth_state.json 已刪除"
...
if [ "$RESULT" = "302" ]; then
    ...
    echo ""
    echo "回應內容:"
    grep -i "錯誤\|error\|失敗\|鎖定" /tmp/login_response.html | head -5
    echo ""
    echo "Server log:"
