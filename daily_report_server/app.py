...
# 注意：SEED_USERS 僅在 users.json 不存在時使用
# 實際用戶資料以 data/users.json 為準（含 15 位完整員工）
SEED_USERS = {}  # 已停用，直接使用 users.json
...
def public_user(u: dict) -> dict:
    return {
        ...
        'department': u.get('department', '未分類'),
...
def login():
    ...
    if request.method == 'POST':
        ...
        # 兼容舊登入習慣：20101 視同 admin
        username = 'admin' if raw_username == '20101' else raw_username
